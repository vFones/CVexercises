...
cv::Mat kmeans(cv::Mat src, int clusterSize, int maxIterations)
{
  ...
  for(it = 0; it < maxIterations; it++)
  {
    for(auto i = 0; i < src.rows; i++)
      for(auto j = 0; j < src.cols; j++)
        clusters.at<uchar>(i, j) = whichCluster(centroids, src.at<cv::Vec3b>(i, j));
  ...
  for(int i = 0; i < clusters.rows; i++)
    for(int j = 0; j < clusters.cols; j++)
      dst.at<cv::Vec3b>(i, j) = centroids.at(clusters.at<uchar>(i, j));
